...
basedir='/global/cfs/cdirs/desi/users/cmanser/data/WD_calib'
...
def fit_DA(spectra, plot = False, verbose_output = False):
    ...
    print("\nFirst solution")
    best_T_err = abs(best_T-other_T[0])
    best_g_err = abs(best_g-other_T[1])
    print("T = ", best_T, best_T_err)
    print("logg = ", best_g/100, best_g_err/100)
    print("rv =",best_rv)
    ...
    s_best_T, s_best_g, s_best_rv = sec_best[0][0], sec_best[0][1], sec_best[0][2]
    print("\nSecond solution")
    s_best_T_err = abs(s_best_T-other_T2[0])
    s_best_g_err = abs(s_best_g-other_T2[1])
    print("T = ", s_best_T, s_best_T_err)
    print("logg = ", s_best_g/100, s_best_g_err/100)
    ...
    if plot == True:
        ...
        lines_s,lines_m,mod_n=fit_func((best_T,best_g,best_rv),spec_n,l_crop,mode=1)
        lines_s_o,lines_m_o,mod_n_o=fit_func((s_best_T,s_best_g,s_best_rv),spec_n,
                                             l_crop,mode=1)
        fig=plt.figure(figsize=(8,5))
        ax1 = plt.subplot2grid((1,4), (0, 3))
        step = 0
        for i in range(6): # plots Halpha (i=0) to H6 (i=5)
    ...
    if verbose_output == False:
        ...
        return best_T, best_T_err, best_g, best_g_err, best_rv, s_best_T, s_best_T_err, s_best_g, s_best_g_err, StN
